...
use image::{DynamicImage, ImageBuffer, ImageError, RgbaImage};
...
fn main() {
    ...
    let cmd_res = {
        if matches.subcommand_name().unwrap() == "keygen" {
            ...
            let key_pair_path: PathBuf = key_gen_matches
                .get_one::<String>("path")
                .map(PathBuf::from)
                .unwrap_or(key_dir);
            let key_pair_name: String = key_gen_matches
                .get_one::<String>("name")
            ...
            let public_key_path: Option<&String> =
                decrypt_matches.get_one::<String>("public-key");
            let private_key_path: Option<&String> =
                decrypt_matches.get_one::<String>("private-key");
            ENCRYPTION_CONFIG
                .set(EncryptionConfig {
                    key_dir,
...
fn cli_key_gen(path: &PathBuf, name: &String) -> Result<(), SmartConsoleCLIError> {
    let die_if_exists = |path: &PathBuf| match path.exists() {
        true => Err(SmartConsoleCLIError::KeyPathExists(
...
fn decrypt_from_clipboard() -> Result<Challenge, SmartConsoleCLIError> {
    ...
    let image_data = match cb.get_image() {
        ...
        Err(arboard::Error::ContentNotAvailable) => {
            Err(SmartConsoleCLIError::ClipboardEmpty)
        }
        Err(e) => Err(e.into()),
    }?;
    let width = image_data
        .width
        .try_into()
        .map_err(|e| SmartConsoleCLIError::ImageData("width", e))?;
    let height = image_data
        .height
        .try_into()
        .map_err(|e| SmartConsoleCLIError::ImageData("height", e))?;

    let rgba: RgbaImage =
        ImageBuffer::from_raw(width, height, (*image_data.into_owned_bytes()).to_vec())
            .ok_or(SmartConsoleCLIError::ImageFromRaw)?;
...
fn decrypt_from_image(
    image: DynamicImage,
    width: u32,
    height: u32,
) -> Result<Challenge, SmartConsoleCLIError> {
    ...
    let results = scanner
        .scan_y800(&luma_img_data, width, height)
        .map_err(|msg| SmartConsoleCLIError::Scan(msg.to_string()))?;
...
fn decrypt(content: &str) -> Result<Challenge, SmartConsoleCLIError> {
    ...
    let bytes: [u8; 32] = server_public_key
        .try_into()
        .map_err(|_| SmartConsoleCLIError::PublicKeyFormat)?;
    ...
    for k in decryption_keys {
        ...
        match decryption_box.decrypt(nonce.as_slice().into(), &ciphertext[..]) {
            ...
            Err(error) => {
                let wrapped_error = DecryptionError { key: k.path, error };
                debug!("{:?}", &wrapped_error);
...
fn get_decryption_keys() -> Result<Vec<DecryptionKey>, SmartConsoleCLIError> {
    ...
    let keys_to_read: Vec<PathBuf> = match &config.private_key_path {
        ...
        None => {
            // otherwise scan key-dir for .private files
                    ...
                    e.ok()
                        .map(|ee| {
                            let is_file = ee
                                ...
                                .unwrap_or(false);
                            let file_name =
                                ee.file_name().to_str().unwrap_or("").to_string();
    ...
    Ok(keys_to_read
        .iter()
        .filter_map(|k| match read_key(k) {
            Ok(kk) => Some(kk),
            Err(e) => {
                error!("failed to read key: {:?}, error: {:?} - ignoring", k, e);
                None
            }
        })
        .collect())
...
fn read_key(path: &PathBuf) -> Result<DecryptionKey, SmartConsoleCLIError> {
    ...
    let bytes: [u8; 32] = buffer
        .as_slice()
        .try_into()
        .map_err(|_| SmartConsoleCLIError::PrivateKeyFormat)?;
    Ok(DecryptionKey {
        path: path.to_owned(),
